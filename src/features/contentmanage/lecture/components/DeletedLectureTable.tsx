    ...
    <div className="mt-5 w-full max-w-full overflow-hidden rounded-[20px] border border-[#E4E7EC] bg-white">
      <table className="w-full table-fixed border-collapse">
        <caption className="sr-only">
          국가, 강의 제목, 난이도, 가격, 상태가 포함된 삭제 강의 목록입니다.
        </caption>
        <colgroup>
          <col className="w-[12%]" />
          <col className="w-[13%]" />
        ...
        <thead>
          <tr className="border-b border-[#E4E7EC] bg-[#FCFCFD] text-[13px] font-semibold text-[#667085]">
            ...
            <th scope="col" className="px-3 py-4 text-left">국가 ID</th>
        ...
        <tbody>
          {courses.length === 0 ? (
            <tr>
              <td colSpan={6} className="h-[200px] px-3 py-5 text-center text-[14px] text-[#98A2B3]">
                삭제된 강의가 없습니다.
              </td>
            </tr>
          ) : (
            courses.map((course) => (
              <tr key={course.courseId} className="border-b border-[#F2F4F7] text-[14px] text-[#344054] last:border-b-0">
                ...
                <td className="px-3 py-4 font-medium">{course.countryId}</td>
                <td className="px-3 py-4">
                ...
                <td className="px-3 py-4 text-center">
                  <span className="inline-flex rounded-full bg-[#F2F4F7] px-3 py-1 text-[12px] font-semibold text-[#667085]">
