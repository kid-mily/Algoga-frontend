...
// EXPIRED(2026-07-22 백엔드 추가 — 출발일 지난 미결제 PENDING을 매일 배치로 전환)도 같은 이유로 방어적으로 제외
...
export async function payReservationBalance(
  ...
  try {
    ...
    // 2026-07-22 백엔드 확인 — 위 사전 체크를 통과했어도 서버가 최종적으로 한 번 더 막을 수 있음
    if (errorCode === "BK_005") {
      throw new Error("출발일이 지나 예약·결제할 수 없습니다.");
    }
    if (errorCode === "BK_006") {
      throw new Error("출발 7일 전 잔금 결제가 마감되었습니다.");
    }
    throw error;
  }
}
