...
const getStudyHref = (course: MyCourse) => {
  const continentCode = course.continentCode?.toLowerCase();
  ...
  return (
    <>
      <div className="space-y-4">
        <LatestDiagnosisBanner diagnoses={latestDiagnoses} />
        ...
        <section>
          <div className="mb-3 flex items-end justify-between">
            ...
            <span className="text-xs text-[#8A9BB0]">
              총 {pagination?.totalElements ?? courses.length}개 강의
            </span>
          </div>
              ...
              <ul className="space-y-3">
                {courses.map((course) => {
                  const progress = Math.min(
                    Math.max(Number(course.progressRate ?? 0), 0),
                    100
                  );
              ...
              {pagination && pagination.totalPages > 1 ? (
                ...
                >
                  <button
                    ...
                    className="h-9 rounded-lg border border-[#D9E2EC] bg-white px-3 text-xs font-bold text-[#4A5568] disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    이전
                  </button>
                      ...
                      <button
                        ...
                        className={`h-9 min-w-9 rounded-lg px-3 text-xs font-bold ${
                          page === index
                            ...
                            : "border border-[#D9E2EC] bg-white text-[#4A5568]"
                        }`}
                      >
                        {index + 1}
                      </button>
