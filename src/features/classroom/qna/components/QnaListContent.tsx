...
import { formatQnaDate, getStatusLabel } from "./utils";
...
export default function QnaListContent({ qnas, continentCode, countryid, courseId }: QnaListContentProps) {
    ...
    const filteredQnas = useMemo(() => {
        ...
        return qnas.filter((qna) => {
            return (
                qna.title.toLowerCase().includes(searchKeyword) || qna.question.toLowerCase().includes(searchKeyword)
            );
        });
    }, [keyword, qnas]);
