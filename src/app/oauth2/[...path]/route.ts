// 로컬에서 소셜 로그인(카카오/구글) 되게 하려고 만든 코드 (api/[...path] 프록시와 동일한 방식)
...
async function proxyOAuth2(
  request: NextRequest,
  { params }: { params: Promise<{ path: string[] }> }
) {
  const { path } = await params;
  const requestUrl = new URL(request.url);
  const targetUrl = new URL(`${getApiProxyTarget()}/oauth2/${path.join("/")}`);
