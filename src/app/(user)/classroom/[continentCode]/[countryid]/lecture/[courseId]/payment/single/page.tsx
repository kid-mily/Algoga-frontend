...
import type { MyCoupon } from "@/features/payment/types";
import type { CourseItem } from "@/features/classroom/components/types";

// 동적 라우트 파라미터를 문자열로 변환
const getParam = (
  value: string | string[] | undefined
): string => {
  if (!value) {
...
export default function SingleLecturePaymentPage() {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      let portonePaymentId: string | null =
        null;
       ...
       */
      if (
        error instanceof LecturePaymentError
      ) {
        console.error(
          "🔍 결제 API 오류 상세:",
          {
