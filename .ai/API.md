...
- `AUTH_017`: 다른 기기에서 동일 계정으로 로그인해 기존 세션이 종료된 경우. 사용자에게 다른 기기 로그인 안내 모달을 표시한 뒤 로그인 화면으로 이동한다.
- `AUTH_018`: 30분간 활동이 없어 세션이 만료된 경우. 사용자에게 미활동 세션 만료 안내 모달을 표시한 뒤 로그인 화면으로 이동한다.
- 에러 응답은 `errorCode`를 우선 사용하고, 기존 응답 호환을 위해 `code`도 함께 지원한다.
- 미분류 401은 기존 동작을 유지해 미활동 세션 만료 안내로 처리한다.
...
- 2026-07-24 — 패키지 상세 "패키지 안내" 탭에 패키지 자체 설명(`packageItem.description`, 여행 상품 설명)을 새로 표시. 이 필드는 API 타입(`PackageApiItem.description`)엔 있었지만 `PackageDetailData`로 전달되는 매핑 단계에서 아예 빠져 있어서 화면 어디에도 표시된 적이 없었음(숙소 설명 `accommodationDescription`과는 다른 필드). `PackageDetailData`에 `description: string` 추가하고 `toPackageDetailData`에서 `packageItem.description ?? ""`로 매핑, `PackageGuide.tsx`에 값 있을 때만 렌더되는 카드 추가(빈 값이면 안 보임) / 영향: `src/features/packagelounge/packageDetail.types.ts`, `src/features/packagelounge/packageDetail.util.ts`, `src/features/packagelounge/components/PackageGuide.tsx`
